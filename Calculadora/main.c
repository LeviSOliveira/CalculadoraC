...
double notacaoPosFixa(char *expressao) {
    ...
    while (obtido != NULL){
        if(isdigit(*obtido) || (*obtido == '-' && isdigit(*(obtido + 1)))){
            empilha(&s, atof(obtido));
        } else{
            double val2 = desempilhar(&s);
            double val1 = desempilhar(&s);
            switch (*obtido){
                case '+': empilha(&s, val1 + val2); break;
                case '-': empilha(&s, val1 - val2); break;
                case '*': empilha(&s, val1 * val2); break;
                case '/': 
                    if(val2 == 0){
                    ...
                    empilha(&s, val1 / val2); 
                    break;
                case '^': empilha(&s, pow(val1, val2)); break;
                case 'r': empilha(&s, sqrt(val2)); break; // Raiz quadrada
                case 's': empilha(&s, sin(val2)); break; // Seno
                case 'c': empilha(&s, cos(val2)); break; // Cosseno
                case 't': empilha(&s, tan(val2)); break; // Tangente
                case 'l': empilha(&s, log10(val2)); break; // Logaritmo base 10
                default:
